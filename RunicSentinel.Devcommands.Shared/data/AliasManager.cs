...
{
  public static string Pattern = "RunicSentinel.alias*.yaml";
  public static string Folder = "RunicSentinel.alias";
  public const string DefaultFile = "RunicSentinel.alias.yaml";
  ...
  public static void AddCommand(string key, string value)
  {
    ...
    if (Terminal.commands.TryGetValue(baseCommand, out var command))
      new Terminal.ConsoleCommand(key, plain, command.action, command.IsCheat, command.IsNetwork, command.OnlyServer, command.IsSecret, command.AllowInDevBuild, optionsFetcher: command.m_tabOptionsFetcher);
    else
