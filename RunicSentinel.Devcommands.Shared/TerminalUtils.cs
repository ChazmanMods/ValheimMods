...
public static class TerminalUtils
{
  ...
  public static bool TryResolveInputPlaceholders(Terminal t,string command)=>false;
  public static string GetLastWord(Terminal obj)=>"";
#else
...
public class TryRunCommand
{
  ...
  {
    if (!Settings.ImprovedChat && __instance == Chat.instance) { SentinelBridge.Submit(__instance,text);return false; }
    RemoveWhitespacedFromHistory(__instance);
    // Some commands (like alias or bind) are expected to be executed as they are.
    if (TerminalUtils.SkipProcessing(text)) { SentinelBridge.Submit(__instance,text);return false; }
  ...
  internal static string CheckLogic(string text)
  {
  ...
  static void RunCommand(Terminal t, string text)
  {
    ...
    if(ZNet.instance!=null&&ZNet.instance.IsServer())cmd.RunAction(new Terminal.ConsoleEventArgs(text,t,cmd));
    else SentinelBridge.Submit(t,text);
  }
}
...
public class RunAction
{
  static void Prefix(Terminal.ConsoleEventArgs args)
  {
    ...
    for (var i = 1; i < args.Args.Length; i++)
    {
      ...
      {
        if (option != null && option.Equals(args.Args[i], StringComparison.OrdinalIgnoreCase))
        {
          args.Args[i] = option;
          break;
        }
