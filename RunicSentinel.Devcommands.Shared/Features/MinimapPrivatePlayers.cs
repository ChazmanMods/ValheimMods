...
public class SendPrivatePositionsToAdmins
{
  ...
  private static void SendToAdmins(ZNet obj)
  {
    ...
    if (count == 0) return;
    var peers = obj.m_peers.Where(peer => peer.IsReady() && SentinelBridge.AllowRpc(peer.m_rpc)).ToList();
...
public class RegisterRpcPrivatePositions
{
  ...
  {
    if (!Settings.ShowPrivatePlayers||!ReferenceEquals(rpc,ZNet.instance.GetServerRPC())) return;
    var length = pkg.ReadInt();
    if(length<0||length>256)return;
    PrivatePositions.Clear();
  ...
  {
    if (__instance.IsServer()||!ReferenceEquals(rpc,__instance.GetServerRPC())) return;
