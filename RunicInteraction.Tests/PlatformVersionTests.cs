    ...
    internal static class PlatformVersionTests
    {
        ...
        private static void DisplayPrefixesDoNotReject()
        {
            ...
            foreach (string label in new[] { build, "l-" + build, "platform-" + build })
            {
                ...
                ValheimAccess.ValidateGameVersion(typeof(FakeVersion));
            }
            TestAssert.Equal(0, FakeVersion.DisplayReads);
            ValheimAccess.ValidateGameVersion(typeof(Player).Assembly.GetType("Version", true));
        }
        ...
        private static void UnsupportedBuildsReject()
        {
            ...
            {
                FakeVersion.Value = build;
                ExpectRejected(() => ValheimAccess.ValidateGameVersion(typeof(FakeVersion)));
            }
        }

        private static void MissingContractRejects()
        {
            ExpectRejected(() => ValheimAccess.ValidateGameVersion(typeof(DisplayOnlyVersion)));
            ExpectRejected(() => ValheimAccess.ValidateGameVersion(null));
        }
