...
if ($manifest.name -cne 'RunicDisplayStands' -or $version -notmatch '^\d+\.\d+\.\d+$' -or
...
$sources = [ordered]@{
    ...
    'RunicDisplayStands.dll' = Join-Path $module 'bin/Release/netstandard2.1/RunicDisplayStands.dll'
    'RunicDisplayStands.cfg.example' = Join-Path $module 'RunicDisplayStands.cfg.example'
}
$hashes = [ordered]@{}
foreach ($entry in $sources.GetEnumerator()) {
    $hashes[$entry.Key] = (Get-FileHash -LiteralPath $entry.Value -Algorithm SHA256).Hash
}
$identity = [Reflection.AssemblyName]::GetAssemblyName($sources['RunicDisplayStands.dll'])
...
if ((Get-Content -Raw -LiteralPath (Join-Path $module 'Plugin.cs')) -notmatch
    ('const string PluginVersion = "' + [regex]::Escape($version) + '"')) { throw 'Plugin version mismatch.' }
...
$auditDir = Join-Path $repo "artifacts/Thunderstore/RunicDisplayStands/$version"
New-Item -ItemType Directory -Path $auditDir, $OutputDirectory -Force | Out-Null
$packageName = "Chazman-RunicDisplayStands-$version.zip"
