...
namespace RunicSentinel.Input;

// Unique patch namespace: HarmonyX keys camera __state by declaring type name.
// This guard belongs to Sentinel and requires no other Runic mod.
internal static class ModalGameplayInput
{
    ...
    private static bool _awaitingRelease;
    private static readonly string[] HeldActions = { "Attack", "JoyAttack", "SecondaryAttack", "JoySecondaryAttack", "Block", "JoyBlock" };
    internal static void CaptureOpen()
    {
    ...
    internal static bool Blocked {
        get {
            if (IsOpen?.Invoke() == true) { CaptureOpen(); return true; }
            if (_awaitingRelease) {
                // Read raw state, not the getters patched below. A closing click/held
                // controller trigger must be released before gameplay can resume.
                foreach (string action in HeldActions)
                    if (ZInput.instance?.GetButtonDef(action)?.Held == true) return true;
    ...
    internal static void Reset() { IsOpen = null; _lastOpenFrame = -10; _cameraDepth = 0; _awaitingRelease = false; }
...
internal static class ModalShortcutKeyPatch
{
    ...
    {
        var shortcut = SentinelConfig.AdminPanelKey?.Value ?? new BepInEx.Configuration.KeyboardShortcut(KeyCode.F3);
        if (key == shortcut.MainKey || shortcut.Modifiers.Contains(key)) return;
