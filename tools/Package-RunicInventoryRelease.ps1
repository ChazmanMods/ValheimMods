...
$module = Join-Path $repo 'RunicInventory'
...
if ($manifest.name -cne 'RunicInventory' -or $version -notmatch '^\d+\.\d+\.\d+$' -or
...
$sources = [ordered]@{
    ...
    'RunicInventory.dll' = Join-Path $module 'bin/Release/netstandard2.1/RunicInventory.dll'
    'RunicInventory.cfg.example' = Join-Path $module 'RunicInventory.cfg.example'
}
$hashes = [ordered]@{}
foreach ($entry in $sources.GetEnumerator()) {
    $hashes[$entry.Key] = (Get-FileHash -LiteralPath $entry.Value -Algorithm SHA256).Hash
}
$identity = [Reflection.AssemblyName]::GetAssemblyName($sources['RunicInventory.dll'])
...
$auditDir = Join-Path $repo "artifacts/Thunderstore/RunicInventory/$version"
New-Item -ItemType Directory -Path $auditDir, $OutputDirectory -Force | Out-Null
$packageName = "Chazman-RunicInventory-$version.zip"
