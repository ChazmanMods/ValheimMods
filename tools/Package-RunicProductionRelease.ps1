...
$module = Join-Path $repo 'RunicProduction'
...
if ($manifest.name -cne 'RunicProduction' -or $version -notmatch '^\d+\.\d+\.\d+$' -or
...
$sources = [ordered]@{
    ...
    'RunicProduction.dll' = Join-Path $module 'bin/Release/netstandard2.1/RunicProduction.dll'
    'RunicProduction.cfg.example' = Join-Path $module 'RunicProduction.cfg.example'
}
$hashes = [ordered]@{}
...
$auditDir = Join-Path $repo "artifacts/Thunderstore/RunicProduction/$version"
New-Item -ItemType Directory -Path $auditDir, $OutputDirectory -Force | Out-Null
$packageName = "Chazman-RunicProduction-$version.zip"
