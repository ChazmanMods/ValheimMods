...
namespace QuietBuildRotation.Tests
{
    ...
    internal static class IlReader
    {
        ...
        internal static IReadOnlyList<IlInstruction> Read(MethodBase method)
        {
            ...
            TestAssert.True(body != null, method + " has no IL body.");
            byte[] bytes = body.GetILAsByteArray();
            TestAssert.True(bytes != null, method + " has no IL bytes.");
            ...
            while (index < bytes.Length)
            {
                ...
                TestAssert.True(!string.IsNullOrEmpty(code.Name), "Unknown IL opcode at " + offset + ".");
        ...
        internal static int CountCalls(MethodBase method, Type owner, string name) =>
            Calls(method).Count(call => call.DeclaringType == owner && call.Name == name);
